      ...
      for (var i = 0; i< 999; i++) {
          ...
          .getText('td.fnstatus > table > tbody > tr > td', function(result){
            if(result.value.toLowerCase() !== browser.globals.status.toLowerCase()){
              ...
              .setValue('input#Email', process.env.FROM_EMAIL)
              .click('input#next')
              .waitForElementVisible('input#Passwd', 1000)
              .setValue('input#Passwd', process.env.PASSSWORD)
              .click('input#signIn')
              .pause(6000)
              .useXpath()
              ...
              .setValue('//textarea[@name="to"]', `${process.env.FROM_EMAIL}; ${process.env.TO_EMAIL};`)
